...
const ArtworkDetail = () => {
  ...
  const fetchArtwork = async (artworkId: string) => {
    try {
      ...
      const sortedArtwork = {
        ...data,
        artwork_images: data.artwork_images.sort((a: ArtworkImage, b: ArtworkImage) => a.display_order - b.display_order)
      };
  ...
  const nextImage = () => {
    if (artwork && artwork.artwork_images.length > 1) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <p className="text-white">{t('gallery.loadingArtworks')}</p>
      </div>
    );
  }
